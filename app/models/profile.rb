...
class Profile < ApplicationRecord
  ...
  # Whitelist for `first_name` and `last_name`. Allows unicode letters/digits,
  # whitespace, and the punctuation actually found in Brazilian names:
  # ` ' . - , ( ) `. Explicitly rejects HTML delimiters (`<`, `>`, `&`, `"`,
  # `/`) — closes the XSS Stored vector found in the 2026-05-17 pentest
  # (item #22 in doc/security/pentest-2026-05-17.md).
  #
  # Limited to first_name + last_name because those were the fields the
  # pentest actually exercised. Other short fields (social_name, mother_name)
  # can be hardened in a future round if they show up in a report.
